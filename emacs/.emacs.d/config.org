...
** AUCTeX
   ...
   before loading outline mode. This includes anything that is built
   on top of outline mode, e.g. Org mode. Otherwise the keymap has to
   be modified.
...
* Some other ideas:
  ...
  - company-reftex   :: backend based on RefTeX, this is the standard
       in emacs. Maybe a better solution than bibtex only. Seems very
       powerful, but AucTeX centered.
